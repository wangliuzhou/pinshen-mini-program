...
Component({
  ...
  methods: {
    ...
    selectDay(e) {
      ...
      let time = canlender.year + "-" + month + "-" + date
      if (moment(Date()).isAfter(moment(time))) {
        this.getWeek(time);
      }
    },
    packup() {
    ...
    dataBefor(e) {
      ...
      var moment = require("moment");
      console.log(moment(Date()).isAfter(moment(_date)));
      console.log(num);
      if (num == -1 || moment(Date()).isAfter(moment(_date))) {
        this.getWeek(_date);
      }
    },
    // 获取日历内容
